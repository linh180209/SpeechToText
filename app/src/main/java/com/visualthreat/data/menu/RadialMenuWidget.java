...
public class RadialMenuWidget extends View {
	...
	public void Set_OnTouch(OnTouch m){
		this.mOnTouch = m;
	}
				...
				for (int i = 0; i < Wedges.length; i++) {
					...
					if (f == selected) {
						...
						if (enabled != null) {
							...
							menuEntries.get(i).menuActiviated(menuEntries.get(i).getName());
	...
	protected void onDraw(Canvas c) {
		...
		for (int i = 0; i < Wedges.length; i++) {
			//Log.d("test", "i: " + i);
			Shader radialGradientShader = radialGradientShader = new RadialGradient(
					xPosition, yPosition, 1500, Color.BLACK, Color.WHITE,
			...
			if ((menuEntries.get(i).getIcon() != 0) && (menuEntries.get(i).getLabel() != null)) {
				...
				Rect rf2 = new Rect();
				rf2.set(rf.left+2, rf.top-((int)textHeight/2)+2, rf.right-2, rf.bottom-((int)textHeight/2)-2);
		...
		//Log.d("test","x: " + xPosition);
		//Log.d("test","y: " + yPosition);
		if (inCircle == true) {
			Log.d("test","press miss");
			paint.setColor(selectedColor);
			paint.setAlpha(selectedAlpha);
			paint.setShader(radialGradientShader);
			paint.setStyle(Paint.Style.FILL);
		...
		if ((centerCircle.getIcon() != 0) && (centerCircle.getLabel() != null)) {
			...
			int h = getIconSize(drawable.getIntrinsicHeight(),MinIconSize + 30,MaxIconSize + 30);
			int w = getIconSize(drawable.getIntrinsicWidth(),MinIconSize + 30,MaxIconSize + 30);
	...
	public int scalePX( int dp_size )
	{
